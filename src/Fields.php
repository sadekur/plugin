...
abstract class Fields extends Base {
	...
	public function hooks() {
		if( did_action( "cv-plugin_{$this->config['id']}_loaded" ) ) return;
		do_action( "cv-plugin_{$this->config['id']}_loaded" );
						...
						if( isset( $section['fields'] ) && is_array( $section['fields'] ) && count( $section['fields'] ) > 0 ) {
							foreach ( $section['fields'] as $field ) {
								if( isset( $field['condition'] ) && is_array( $field['condition'] ) ) {
									...
									if( 'checked' != $compare ) {
										echo "$('#{$section['id']}-{$key}').change(function(e){if( $('#{$section['id']}-{$key}').val() {$compare} '{$value}' ) { $('#cv-row-{$section['id']}-{$field['id']}').slideDown();}else { $('#cv-row-{$section['id']}-{$field['id']}').slideUp();}}).change();";
									}
									else {
										echo "$('#{$section['id']}-{$key}').change(function(e){if( $('#{$section['id']}-{$key}').is(':checked') ) { $('#cv-row-{$section['id']}-{$field['id']}').slideDown();}else { $('#cv-row-{$section['id']}-{$field['id']}').slideUp();}}).change();";
									}
								}
							}
						}
	...
	public function callback_fields( $post = null, $metabox = [] ) {
		...
		echo "<h2 class='cv-heading'>{$icon} {$config['title']}</h2>";
		endif;

		do_action( 'cv-settings-heading', $config );

		if( ! isset( $this->sections ) || count( $this->sections ) <= 0 ) return;

		$tab_position = isset( $config['topnav'] ) && $config['topnav'] == true ? 'top' : 'left';
		
		echo "<div class='cv-wrapper-outer'>
				<div class='cv-wrapper cv-shadow cv-tab-{$tab_position} cv-sections-" . count( $this->sections ) . "'>";
		...
		echo '
		<div class="cv-navs-wrapper" style="display: ' . $display . '">
			<ul class="cv-nav-tabs">';
			foreach ( $sections as $section ) {
				$icon = $this->generate_icon( $section['icon'] );
				$color = isset( $section['color'] ) ? $section['color'] : '#1c2327';
				echo "<li id='cv-nav-tab-{$section['id']}' class='cv-nav-tab' data-color='{$color}'><a href='#{$section['id']}'>{$icon}<span id='cv-nav-label-{$section['id']}' class='cv-nav-label'> {$section['label']}</span></a></li>";
			}
			echo '</ul>
		</div><!--div class="cv-navs-wrapper"-->';

		// form areas
		echo '<div class="cv-sections-wrapper">';
		foreach ( $sections as $section ) {
			$icon = $this->generate_icon( $section['icon'] );
			$color = isset( $section['color'] ) ? $section['color'] : '#1c2327';
			...
			echo "<div id='{$section['id']}' class='cv-section' style='display:none'>";

			do_action( 'cv-settings-before-form', $section );

			$fields = isset( $section['fields'] ) ? $section['fields'] : [];
			$fields = apply_filters( 'cv-settings-fields', $fields, $section );
			$show_form = isset( $section['hide_form'] ) && $section['hide_form'] ? false : true;
			$show_form = apply_filters( 'cv-settigns-show-form', $show_form, $section );

			if( $scope == 'option' && $show_form ) {
				$page_load = isset( $section['page_load'] ) && $section['page_load'] ? 1 : 0;

				echo "<form id='cv-form-{$section['id']}' class='cv-form'>
						<div id='cv-message-{$section['id']}' class='cv-message'>
							<img src='" . plugins_url( 'assets/img/checked.png', __FILE__ ) . "' />
							<p></p>
						</div>
						<input type='hidden' name='action' value='cv-settings' />
			...
			if( ! isset( $section['no_heading'] ) || $section['no_heading'] !== true ) {
				echo "<div class='cv-subheading'>";
				
				do_action( 'cv-settings-before-title', $section );
			
				echo "<div class='cv-section-subheading' style='color: {$color}'>{$icon}</span> <span class='cv-subheading-text'>{$section['label']}</div>";
			
				do_action( 'cv-settings-after-title', $section );

				if( $show_form && ( ! isset( $section['top_btn'] ) || false !== $section['top_btn'] ) ) {
					echo "<div id='cv-section-top_btn-{$section['id']}' class='cv-section-top_btn'>";
					if( $reset_button ) echo "<button type='button' class='button button-hero cv-reset-button' data-option_name='{$section['id']}' data-_nonce='{$_nonce}'>{$reset_button}</button>&nbsp;";
					if( $submit_button ) echo "<input type='submit' class='button button-hero button-primary cv-submit' value='{$submit_button}' />";
					echo '</div>';
				}
			...
			if( isset( $section['desc'] ) && $section['desc'] != '' ) {
				echo "<p class='cv-desc'>{$section['desc']}</p>";
			}

			do_action( 'cv-settings-before-fields', $section );
			...
			do_action( 'cv-settings-after-fields', $section );

			if( $scope == 'option' && $show_form ) {
				$_is_sticky = isset( $section['sticky'] ) && ! $section['sticky'] ? ' cv-nonsticky-controls' : ' cv-sticky-controls';
				echo "<div class='cv-controls-wrapper{$_is_sticky}'>";

				if( $reset_button ) echo "<button type='button' class='button button-hero cv-reset-button' data-option_name='{$section['id']}' data-_nonce='{$_nonce}'>{$reset_button}</button>&nbsp;";
				if( $submit_button ) echo "<input type='submit' class='button button-hero button-primary cv-submit' value='{$submit_button}' />";

				echo '</div class="cv-controls-wrapper">
				</form>';
			}

			do_action( 'cv-settings-after-form', $section );

			echo "</div><!--div id='{$section['id']}'-->";
		}

		echo '</div><!--div class="cv-sections-wrapper"-->
			 <div class="cv-sidebar-wrapper">';

		do_action( 'cv-settings-sidebar', $config );

		echo '</div><!--div class="cv-sidebar-wrapper"-->
			</div><!--div class="cv-wrapper"-->';

		do_action( 'cv-settings-after_wrapper', $config );

		echo '</div><!--div class="cv-wrapper-outer"-->
		</div><!--div class="wrap"-->
		<div id="cv-overlay" style="display: none;">
	...
	public function populate_fields( $fields, $section, $scope ) {
		...
		foreach ( $fields as $field ) {
			...
			do_action( 'cv-settings-before-row', $field, $section );

			$_show_label = isset( $field['label'] ) && $field['type'] != 'tabs';

			if( isset( $field['type'] ) && $field['type'] == 'divider' ) {
				$style = isset( $field['style'] ) ? $field['style'] : '';
				echo "<div class='cv-row cv-divider' id='{$section['id']}-{$field['id']}' style='{$style}'><span>{$field['label']}</span></div>";
			}
			else {
				$field_display = isset( $field['condition'] ) && is_array( $field['condition'] ) ? 'none' : '';
				echo "
				<div id='cv-row-{$section['id']}-{$field['id']}' class='cv-row cv-row-{$section['id']} cv-row-{$field['type']}' style='display: {$field_display}'>";

				if( $_show_label ) {
					echo "<div class='cv-label-wrap'>";

					do_action( 'cv-settings-before-label', $field, $section );

					echo "<label for='{$section['id']}-{$field['id']}'>{$field['label']}</label>";

					do_action( 'cv-settings-after-label', $field, $section );

					echo "</div>";
				}

				$_label_class = $_show_label ? '' : 'cv-field-wrap-nolabel';
				
				echo "<div class='cv-field-wrap {$_label_class}'>";

					do_action( 'cv-settings-before-field', $field, $section );
					...
					do_action( 'cv-settings-after-field', $field, $section );

					if( isset( $field['desc'] ) && $field['desc'] != '' ) {
						echo "<p class='cv-desc'>{$field['desc']}</p>";
					}

				do_action( 'cv-settings-after-description', $field, $section );

				echo "</div>
				</div>";
			}
			
			do_action( 'cv-settings-after-row', $field, $section );
		}
	...
	public function populate( $field, $section, $scope = 'option' ) {
		...
		return __( 'Invalid field type', 'cv-plugin' );
	}
	...
	public function field_text( $field, $section, $scope ) {
		...
		$class 			= "cv-field cv-field-{$field['type']}";
	...
	public function field_switch( $field, $section, $scope ) {
		...
		if( $multiple ) {
			foreach ( $options as $key => $title ) {
				$html .= "
					<label class='cv-toggle'>
						<input type='checkbox' name='{$name}[]' id='{$id}-{$key}' class='cv-toggle-checkbox {$class}' value='{$key}' {$required} {$disabled} " . ( in_array( $key, (array)$value ) ? 'checked' : '' ) . "/>
						<div class='cv-toggle-switch'></div>
						<span class='cv-toggle-label'>{$title}</span>
					</label>
				";
			}
		}
			...
			$html .= "
				<label class='cv-toggle'>
					<input type='checkbox' name='{$name}' id='{$id}' class='cv-toggle-checkbox {$class}' value='on' {$required} {$disabled} " . checked( $value, 'on', false ) . "/>
					<div class='cv-toggle-switch'></div>
				</label>
			";
		}
	...
	public function field_select( $field, $section, $scope ) {
		...
		$class 			= "cv-field cv-field-{$field['type']}";
		$class 			.= isset( $field['class'] ) ? $field['class'] : '';
		$class 			.= isset( $field['select2'] ) && $field['select2'] ? ' cv-select2' : '';
		$class 			.= isset( $field['chosen'] ) && $field['chosen'] ? ' cv-chosen' : '';
	...
	public function field_file( $field, $section, $scope ) {
		...
		$html  = '';
		$html .= "<input type='text' class='{$class} cv-file' id='{$id}' name='{$name}' value='{$value}' placeholder='{$placeholder}' {$readonly} {$required} {$disabled}/>";
		$html  .= "<input type='button' class='button cv-browse' data-title='{$label}' data-select-text='{$select_button}' value='{$upload_button}' {$required} {$disabled} />";
	...
	public function field_tabs( $field, $section, $scope ) {
		...
		foreach ( $tabs as $id => $tab ) {
			$btn_active		= $count == 0 ? 'cv-tab-active' : '';
			$cnt_display	= $count == 0 ? '' : 'none';

			$buttons .= "<a class='cv-tab {$btn_active}' data-target='cv-tab-{$section['id']}-{$id}'>{$tab['label']}</a>";

			$content .= "<div class='cv-tab-content' id='cv-tab-{$section['id']}-{$id}' style='display: {$cnt_display}'>";
		...
		$style = "<style>
			...
			.cv-tab-active,.cv-tab-active:hover {
				color: {$section['color']};
			}
		</style>";

		$html .= '<nav class="cv-tabs">' . $buttons . '</nav>';
		$html .= $content;
		$html .= $style;
	...
	public function field_repeater( $field, $section, $scope ) {
		...
		for( $i = 0; $i < ( is_array( reset( $values ) ) ? count( reset( $values ) ) : 1 ); $i++ ) {
			...
			$html .= '<button type="button" class="cv-repeater-remove">-</button>';
			$html .= '<button type="button" class="cv-repeater-add">+</button>';
	...
	public function generate_icon( $value ) {
		...
		if( strpos( $value, '://' ) !== false ) {
			return "<img class='cv-icon-{$this->config['id']}' src='{$value}' />";
		}
